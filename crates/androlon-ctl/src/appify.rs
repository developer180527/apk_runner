//! `androlon-ctl appify <apk>` — turn an APK into a real macOS `.app` bundle.
//!
...
//!
//! Name/icon come from `aapt2 dump badging` + extracting the densest PNG/WebP
...
pub fn cmd_appify(cfg: &SdkConfig, args: &[String]) -> androlon_core::Result<()> {
    ...
    let badging = run_capture(&cfg.aapt2(), &["dump", "badging", apk_arg])?;
    ...
    match adb.install(&apk) {
        Ok(()) => println!("✓ installed into the Android runtime"),
        Err(e) => println!("⚠ not installed now ({e}) — will need installing before launch"),
    }

    // ---- 3. Build the bundle skeleton ----
    ...
    // Executable: a copy of androlon-app (same dir as this binary), named
    // after the app so the process — and thus the menu bar — carries its name.
    let exe_name = label.replace('/', "-");
    let androlon_app = std::env::current_exe()
        .map_err(io_err("locate androlon-ctl"))?
        .with_file_name("androlon-app");
    if !androlon_app.exists() {
        return Err(EngineError::SdkMissing(format!(
            "androlon-app binary (expected next to androlon-ctl at {})",
            androlon_app.display()
        )));
    }
    let exe_dest = macos_dir.join(&exe_name);
    std::fs::copy(&androlon_app, &exe_dest).map_err(io_err("copy androlon-app"))?;

    // ---- 4. Icon: densest raster icon in the APK → .icns ----
    let icon_ok = build_icns(&badging, &apk, &res_dir).unwrap_or_else(|e| {
        println!("⚠ icon skipped: {e}");
        false
    });

    // ---- 5. Info.plist: identity + LSEnvironment configuration ----
    let sdk_root = std::fs::canonicalize(&cfg.sdk_root)
        .unwrap_or_else(|_| cfg.sdk_root.clone());
...
"#,
    ...
    println!("  double-click it (or `open '{}'`) to launch {label}", bundle.display());
    Ok(())
}
...
fn build_icns(badging: &str, apk: &Path, res_dir: &Path) -> Result<bool, String> {
    ...
    if let Some(icon_path) = icon_path {
        ...
        let entries = std::fs::read_dir(rasters.join("res")).map_err(|_| "no raster resources in APK".to_string())?;
...
fn run_capture(tool: &Path, args: &[&str]) -> androlon_core::Result<String> {
